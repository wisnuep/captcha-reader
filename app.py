...
def main():
    ...
    uploaded_file = st.file_uploader("Pilih gambar CAPTCHA", type=["jpg", "jpeg", "png"])
    ...
    if uploaded_file is not None:
        ...
        st.metric(label="Prediksi CAPTCHA", value=label, delta=f"{overall:.1f}% confidence")

        st.write("**Confidence per karakter:**")
        cols = st.columns(len(label))
        for col, ch, conf in zip(cols, label, confidences):
            with col:
                st.write(f"`{ch}`")
